...
group by city )
select city, total_spend, percentage_contribution from ( 
select *, dense_rank() over(order by total_spend desc ) as rn, 
round(100*(total_spend/ sum(total_spend) over()),2) as percentage_contribution
from cte ) a
where rn <=5 ;
...
select card_type, yt as year, mt as month, total_spend  
from (select *, rank() over(partition by card_type order by total_spend desc) as rn from cte) a 
where rn=1
...
from credit_card_transcations )
select * from (
select *, row_number() over(partition by card_type order by total_spend ) rn from cte  
...
select *, 100*( total_spend / sum(total_spend) over(partition by city)) as percentage_spend  from cte) a
...
max(case when rn_asc=1 then exp_type end) as lowest_exp_type from
(select *
...
(
select *
from (select city, exp_type, sum(amount) as total_spend, rank() over(partition by city order by sum(amount) desc) rn
from credit_card_transcations
group by city, exp_type)cc
where rn=1) a
inner join 
(select *
from (select city, exp_type, sum(amount) as total_spend, rank() over(partition by city order by sum(amount) ) rn
from credit_card_transcations
group by city, exp_type)dd
where rn=1 ) b
on a.city=b.city

--6- write a query to find percentage contribution of spends by females for each expense type
select exp_type,
100*sum(case when gender='F' then amount else 0 end)/sum(amount) as percentage_female_contribution
from credit_card_transcations
group by exp_type
